export default class GptSearch implements ICommand {
    ...
    public static async behaviour() {
        ...
        if (!tab) {
            tab = await browser.tabs.create({ url: "https://chat.openai.com" });

            preface = "For this question and the ones to follow in this chat thread, please keep in mind the following:\n";
